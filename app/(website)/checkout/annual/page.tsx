...
export default function AnnualCheckoutPage() {
    ...
    const buildCustomId = (telegramUserId: string | null) => {
        ...
        const base = `SNTTRADES_ANNUAL_PLAN${suffix}`;
        if (telegramUserId) {
        ...
        const setupOutsetaSuccessHandler = () => {
            window.addEventListener('message', (event) => {
                if (event.data && event.data.type === 'outseta_success') {
                    ...
                    const redirectUrl = buildThankYouUrl(
                        `/thank-you-3?source=outseta&provider=outseta&product=annual&transaction_id=${encodeURIComponent(event.data.transactionId || 'unknown')}`,
                        telegramUserId
                    );
            ...
            const checkOutsetaSuccess = () => {
                ...
                if (currentUrl.includes('success') || currentUrl.includes('completed')) {
                    ...
                    const telegramUserId = localStorage.getItem('telegram_user_id') || sessionStorage.getItem('telegram_user_id');
                    const redirectUrl = buildThankYouUrl(
                        '/thank-you-3?source=outseta_url&provider=outseta&product=annual',
                        telegramUserId
                    );
                    window.location.href = redirectUrl;
                }
                ...
                if (outsetaWidget) {
                    ...
                    if (widgetText.includes('erfolgreich') || 
                        ...
                        setTimeout(() => {
                            const telegramUserId = localStorage.getItem('telegram_user_id') || sessionStorage.getItem('telegram_user_id');
                            const redirectUrl = buildThankYouUrl(
                                '/thank-you-3?source=outseta_text&provider=outseta&product=annual',
                                telegramUserId
                            );
                            window.location.href = redirectUrl;
                        }, 2000);
            ...
            if ((window as any).paypal) {
                (window as any).paypal.Buttons({
                    ...
                    onApprove: function(data: any) {
                        ...
                        const telegramUserId = localStorage.getItem('telegram_user_id') || sessionStorage.getItem('telegram_user_id');
                        const redirectUrl = buildThankYouUrl(
                            '/thank-you-3?source=paypal_subscription&provider=paypal&product=annual&subscription_id=' + (data.subscriptionID || 'unknown'),
                            telegramUserId
                        );
    ...
    return (
        <>
            <CheckoutTracker product="annual" />
            <style jsx global>{`
                .item-header {
            ...
            >
                ...
                >
                    ...
                    >
                        ...
                        <Text fontSize="xs" color="gray.600">Jährlich abgerechnet</Text>
                    </Flex>
                </Box>
            </Box>
            ...
            <Flex direction={{ base: 'column', md: 'row' }} maxW="6xl" mx="auto" w="full" gap={8} mt={{ base: 0, md: 32 }} px={4}>
                ...
                <section style={{ flex: 1, maxWidth: 520, minWidth: 0 }} className="bg-white rounded-xl shadow-lg p-8 flex flex-col justify-center mx-auto border-2 border-gray-200">
                    ...
                    <div id="paypal-express-section" style={{ marginBottom: "2rem" }}>
                        {/* PayPal Abo Button (Jährlich) */}
                        <div id="annual-paypal-section" style={{ textAlign: "center" }}>
                            {isClient && (
                    ...
                    <Box bg="white"
                        ...
                        <Box as="ul" display="flex" flexDirection="column" gap={2} mb={2} pl={0}>
                            ...
                            <Flex as="li" align="center" gap={2}>
                                ...
                                <Text color="gray.800" as="span">
                                    <Text as="span" fontWeight="semibold" color="gray.900">Maximale Ersparnis</Text> im Vergleich zu anderen Plänen
                                </Text>
                            </Flex>
                        </Box>
                    </Box>
