...
import { Box, VStack, Heading, Text, Stack, HStack, SimpleGrid, Container, } from "@chakra-ui/react";
import SntHero from "@/components/hero/snt-hero";
import { CheckCircle, } from "@phosphor-icons/react/dist/ssr";
import { FiTrendingDown, FiAlertTriangle, FiBarChart2, FiArrowDown } from 'react-icons/fi';
...
export default function RegisterPage() {
    ...
    useEffect(() => {
        ...
        const getEmailFromInput = () => {
            const emailInput = document.querySelector('input[name="Person.Email"], input[type="email"]');
        ...
        const extractEmailFromWidget = () => {
            ...
            if (window.Outseta?.lastSubmittedEmail) {
                console.log('E-Mail gefunden in Outseta.lastSubmittedEmail:', window.Outseta.lastSubmittedEmail);
                return window.Outseta.lastSubmittedEmail;
            }
            return null;
        };
        ...
        const handleOutsetaSuccess = async (event) => {
        ...
        let checkUrlChangeInterval = null;
    ...
    return (<>
      <Box id="register-page" position="relative" bg="white" _before={{
            content: '""',
            position: "absolute",
            inset: 0,
            backgroundImage: "radial-gradient(60% 40% at 50% -10%, rgba(16,185,129,0.18), rgba(0,0,0,0) 70%),radial-gradient(40% 30% at 85% 0%, rgba(34,197,94,0.12), rgba(0,0,0,0) 65%)",
            pointerEvents: "none",
            zIndex: 0,
        }} _after={{
            content: '""',
      ...
      <SntHero onRegisterClick={() => setIsModalOpen(true)}/>
        ...
        <Box as="section" w="full" bg="white" py={{ base: 4, md: 12 }} position="relative" zIndex={1}>
           <Container maxW={{ base: "full", md: "3xl" }}>
                <VStack gap={8} maxW="none" mx="auto">
                    <Box mt={{ base: "0", md: "-24" }} w={{ base: '100%', md: '800px', lg: '1200px', xl: '1400px' }} maxW="100%" mx="auto" mb={{ base: 4, md: 6 }} bg="linear-gradient(120deg, rgba(89, 180, 245, 0.6), rgba(85, 178, 245, 0.3), rgba(239, 68, 68, 0.2), rgba(247, 194, 194, 0.6))" borderRadius="xl" p="7px" position="relative" boxShadow="0 0 40px rgba(6, 140, 239, 0.3), 0 0 40px rgba(239, 68, 68, 0.3), 0 0 0 1px rgba(6, 140, 239, 0.4)">
                      <VStack gap={3}>
                        <Box w="100%" aspectRatio={16 / 9} position="relative" overflow="hidden" bg="black" borderRadius="lg">
                          <BrandedVimeoPlayer videoId="1139395784"/>
                        </Box>
                        
                        {/* Community Stats Element */}
                        <Box p={{ base: 3.5, md: 4 }} w="100%" borderRadius="lg" bg="rgba(0, 0, 0, 0.4)" backdropFilter="blur(12px) saturate(180%)" border={`1px solid rgba(6, 140, 239, 0.3)`}>
                          <Stack direction="row" align="center" gap={{ base: 2, md: 3 }} justify="flex-start" flexWrap="nowrap">
                            <Stack direction="row" gap={-2} flexShrink={0}>
                              <Box w={{ base: 5, md: 6 }} h={{ base: 5, md: 6 }} borderRadius="full" border={`2px solid ${SNT_BLUE}`} overflow="hidden" bg="gray.200" boxShadow="0 4px 8px rgba(0,0,0,0.1)">
                                <img src="/assets/community-stats/user_6819319_6ec853ff-5777-4398-8fcc-06e2621cbcf8.avif" alt="Community member" style={{ width: '100%', height: '100%', objectFit: 'cover' }}/>
                              </Box>
                              <Box w={{ base: 5, md: 6 }} h={{ base: 5, md: 6 }} borderRadius="full" border={`2px solid ${SNT_BLUE}`} overflow="hidden" bg="gray.200" boxShadow="0 4px 8px rgba(0,0,0,0.1)">
                                <img src="/assets/community-stats/4208db19763848b131989eadba9899aa.avif" alt="Community member" style={{ width: '100%', height: '100%', objectFit: 'cover' }}/>
                              </Box>
                              <Box w={{ base: 5, md: 6 }} h={{ base: 5, md: 6 }} borderRadius="full" border={`2px solid ${SNT_BLUE}`} overflow="hidden" bg="gray.200" boxShadow="0 4px 8px rgba(0,0,0,0.1)">
                                <img src="/assets/community-stats/393d1b15978eed96285cf196b2f51eda.avif" alt="Community member" style={{ width: '100%', height: '100%', objectFit: 'cover' }}/>
                              </Box>
                            </Stack>
                            <Text fontSize={{ base: "2xs", md: "xs" }} color="white" textShadow="0 1px 2px rgba(0,0,0,0.3)" textAlign="left" lineHeight="tight" flex="1" minW={0}>
        ...
        <Box as="section" w="full" py={{ base: 12, md: 16 }} px={4} bg="linear-gradient(360deg,rgba(6, 140, 239, 0.2) 0%, rgba(255, 255, 255, 1) 50%, rgba(255, 0, 0, 0.2) 100%)">
          <Container maxW="6xl">
            <VStack gap={{ base: 12, md: 16 }}>
              ...
              <VStack gap={4} w="full">
                <Heading as="h2" size="xl" color="black" textAlign="center">
                  Kommst du an deine
                  <Box as="span" color="black" px="1.5" mx="2" borderRadius="xs" bg="linear-gradient(90deg, rgba(239, 68, 68, 0.6), rgba(239, 68, 68, 0.22) 85%, rgba(239, 68, 68, 0) 100%)">
                   Grenzen?
                  </Box>
                </Heading>
            ...
            { icon: FiBarChart2, title: "Falsches Risk-Management", text: "Ein einziger schlechter Trade kann wochenlange Gewinne ausradieren." },
        ].map((item, i) => (<Box key={i} p={6} bg="rgba(255, 255, 255, 0.6)" borderRadius="xl" border="1px" borderColor="rgba(239, 68, 68, 0.4)" backdropFilter="blur(12px) saturate(180%)" boxShadow="0 4px 6px rgba(0, 0, 0, 0.1)">
               ...
               <VStack gap={8} w="full" maxW="4xl" mx="auto">
                <Heading as="h2" size="2xl" color="black" textAlign="center" fontWeight="bold">
                  Hier sind die{" "}
                  <Box as="span" color="black" px="1.5" borderRadius="xs" bg={`linear-gradient(90deg, ${SNT_YELLOW} 0%, rgba(251, 191, 36,0.22) 85%, rgba(251, 191, 36,0) 100%)`} display="inline-block">
                    Skills
                  </Box>
            ...
            { title: "Skill 3", text: "Strategien für das Management eines kleinen Trading-Kontos und die Anwendung skalierbarer Prinzipien" },
        ].map((item, i) => (<Box key={i} p={6} bg="rgba(255, 255, 255, 0.6)" borderRadius="xl" border="1px" borderColor="rgba(6, 140, 239, 0.4)" backdropFilter="blur(12px) saturate(180%)" boxShadow="0 4px 6px rgba(0, 0, 0, 0.1)">
                        <HStack align="start" gap={4}>
                ...
                {/* FREE BONUS Banner */}
                <Box w="full" bg="black" borderRadius="xl" p={{ base: 6, md: 8 }} mt={8} position="relative" overflow="hidden">
            ...
            { text: "Trading Tools & Indicators" },
        ].map((item, i) => (<Box key={i} w="full" bg="gray.800" borderRadius="md" p={{ base: 4, md: 5 }} cursor="pointer" _hover={{ bg: "gray.700" }} transition="all 0.2s">
                          <HStack gap={3}>
                            <Box flexShrink={0}>
                    ...
                    {/* Large Blue Button */}
                    <Box w="full" bg={SNT_BLUE} borderRadius="md" p={{ base: 5, md: 6 }} cursor="pointer" onClick={() => setIsModalOpen(true)} _hover={{ bg: "#0572c2" }} transition="all 0.2s">
      ...
      {/* Registration Modal */}
      <RegistrationModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} planUid="wmjBBxmV"/>
    </>);
}
