...
export async function POST(request) {
    try {
        ...
        if (!email || !emailListUid) {
            return NextResponse.json({ error: 'E-Mail und E-Mail-Liste UID sind erforderlich' }, { status: 400 });
        }
        ...
        for (let attempt = 0; attempt < 3; attempt++) {
            ...
            if (personSearchResponse.ok) {
                ...
                return NextResponse.json({ error: 'Fehler beim Suchen der Person', details: errorText }, { status: personSearchResponse.status });
            }
        }
        if (!person || !person.Uid) {
            console.warn(`Person mit E-Mail ${email} nicht gefunden nach 3 Versuchen`);
            return NextResponse.json({ error: 'Keine Person mit dieser E-Mail gefunden. Bitte warte einen Moment und versuche es erneut.' }, { status: 404 });
        }
