...
async function validateOutsetaPlan(planUid, outsetaDomain, outsetaApiKey, outsetaSecretKey) {
    try {
...
async function createOutsetaAccount(accountData) {
    ...
    if (accountData.SubscriptionPlan?.Uid) {
        const isPlanValid = await validateOutsetaPlan(accountData.SubscriptionPlan.Uid, outsetaDomain, outsetaApiKey, outsetaSecretKey);
        if (!isPlanValid) {
...
function extractCustomerData(event) {
    try {
