...
import { AbsoluteCenter, Box, Spinner, Text, VStack, Group, } from "@chakra-ui/react";
import { EmptyState } from "@/components/ui/empty-state";
import { Lock, SignIn } from "@phosphor-icons/react/dist/ssr";
import { Profile, Login, SignUp } from "./embed";
function userHasAccessToPlans(plans, user) {
    if (!user?.Account)
        return false;
...
export default function ProtectedRoute({ children, plansWithAccess, }) {
    ...
    const requiredPlans = (() => {
        ...
        const plans = plansWithAccess
            ...
            .map((planName) => {
            const configPlan = projectConfig.auth.plans[planName];
            if (!configPlan) {
            ...
            .filter((p) => p !== null);
        console.log("Required Plans:", plans.map((p) => ({ uid: p.uid, label: p.label })));
        return plans;
    })();
    if (isLoading) {
        console.log("ProtectedRoute: Loading state");
        return (<Box position="relative" h="100vh" w="100vw">
        <AbsoluteCenter>
          <VStack>
            <Spinner color="primary.600" size="xl" borderWidth="4px" css={{ "--spinner-track-color": "colors.neutral.200" }}/>
    ...
    if (!user) {
        ...
        <AbsoluteCenter>
          <VStack>
            <EmptyState icon={<SignIn />} title="Login to continue" description={`This page is available only to ${plansWithAccess
                ? requiredPlans.map((p) => p.label).join(" or ")
                : "registered"} users. To continue, log in to your existing account or sign up.`}>
              <Group>
                <Login popup>
                  <Button>Login</Button>
                </Login>
    ...
    if (!allowAccess) {
        ...
        <AbsoluteCenter>
          <VStack>
            <EmptyState icon={<Lock />} title="Upgrade to unlock" description={`This page is available only to users with a ${requiredPlans[0].label} plan. To continue, please upgrade to a ${requiredPlans[0].label} plan.`}>
