...
export default function SntHero() {
  ...
  useEffect(() => {
    ...
    const createStars = (count: number): Star[] => {
      ...
      for (let i = 0; i < count; i++) {
        stars.push({
          ...
          // leichte Drift in unterschiedliche Richtungen
    ...
    const createGreen = (count: number): Star[] => {
      ...
      for (let i = 0; i < count; i++) {
        parts.push({
          ...
          // leichte vertikale Drift nach oben/unten
    ...
    // grüne Partikel (zwei Layer für Parallax)
    ...
    let t = 0;
    let introAlpha = 0; // für sanftes Einblenden von Nebel/Glow

    // keine Shooting Stars mehr

    const draw = () => {
      ctx.clearRect(0, 0, width, height);

      // kein unterer Nebel mehr (entfernt)
    ...
    <Box position="relative" w="100%" h={{ base: "28vh", md: "40vh" }} background="#000" overflow="hidden">
      ...
      >
        ...
        >
          ...
          >
            KOSTENLOSEN SNT-BOOTCAMP
          </Box>
        </Heading>

        {/* unterer grüner Strich mit Glow */}
        <Box
          w={{ base: "80px", md: "110px" }}
          h={{ base: "1px", md: "2px" }}
          bg={`linear-gradient(90deg, rgba(73,231,156,0) 0%, rgba(73,231,156,0.95) 50%, rgba(73,231,156,0) 100%)`}
