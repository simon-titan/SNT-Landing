...
const AnimatedStat = ({ stat, isInView }) => {
    ...
    return (<VStack gap={2} align="center" justify="center" bg="white" borderRadius="xl" p={6} border="1px solid" borderColor="gray.200" boxShadow="0 8px 32px 0 rgba(0, 0, 0, 0.05)" _hover={{
            transform: "translateY(-4px)",
            boxShadow: "0 12px 40px 0 rgba(59, 130, 246, 0.15)",
            borderColor: "rgba(59, 130, 246, 0.3)"
        }} transition="all 0.3s ease">
      <Text fontSize={{ base: "3xl", md: "5xl" }} fontWeight="bold" color="blue.500">
        {stat.prefix}{count}{stat.suffix}
      </Text>
      <Text fontSize={{ base: "xs", md: "sm" }} color="gray.600" fontWeight="semibold" textAlign="center" textTransform="uppercase" letterSpacing="0.1em" lineHeight="1.3">
        {stat.label}
      </Text>
    </VStack>);
};
export const CourseOverviewSection = () => {
    ...
    return (<Box as="section" py={{ base: 12, md: 20 }} px={{ base: 4, md: 8 }} w="full" display="flex" flexDirection="column" alignItems="center" justifyContent="center" bg="gray.50" position="relative">
      <SimpleGrid ref={ref} columns={{ base: 1, md: 4 }} gap={{ base: 6, md: 8 }} mb={16} w="full" maxW="6xl" position="relative" zIndex={1}>
        {stats.map((stat, i) => (<AnimatedStat key={i} stat={stat} isInView={isInView}/>))}
      </SimpleGrid>
      <Text as="h2" fontSize={{ base: "2xl", md: "4xl" }} fontWeight="bold" textAlign="center" mb={12} color="gray.900" position="relative" zIndex={1}>
      ...
      <SimpleGrid columns={{ base: 1, md: 3 }} gap={8} w="full" maxW="6xl" position="relative" zIndex={1}>
        {courses.map((course, i) => (<Box key={i} bg="white" borderRadius="2xl" boxShadow="0 20px 60px 0 rgba(0, 0, 0, 0.08)" border="1px solid" borderColor="gray.200" p={{ base: 6, md: 8 }} color="gray.900" display="flex" flexDirection="column" minH="400px" position="relative" overflow="hidden" _hover={{
                transform: "translateY(-6px)",
                boxShadow: "0 25px 80px 0 rgba(59, 130, 246, 0.2)",
                borderColor: "rgba(59, 130, 246, 0.4)"
            }} transition="all 0.4s ease">
            <Box mb={4} position="relative" zIndex={1}>
              <Box p={3} borderRadius="xl" bg="rgba(59, 130, 246, 0.15)" border="1px solid rgba(59, 130, 246, 0.3)" display="inline-block" boxShadow="0 4px 12px rgba(59, 130, 246, 0.2)">
                {course.icon}
              </Box>
            </Box>
            <Text fontWeight="bold" fontSize={{ base: "xl", md: "2xl" }} mb={4} color="gray.900" lineHeight="tight" position="relative" zIndex={1}>
              {course.title}
            </Text>
            <Text fontSize="md" mb={6} color="gray.600" lineHeight="1.6" position="relative" zIndex={1}>
              {course.desc}
            </Text>
            <Box mt="auto" position="relative" zIndex={1}>
              <a href="/checkout/lifetime" style={{
                fontWeight: 600,
                color: '#3b82f6',
