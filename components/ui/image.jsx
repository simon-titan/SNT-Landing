...
import { Image as ChakraImage, defaultSystem, } from "@chakra-ui/react";
export const Image = ({ src, alt, width, height, fill, sizes, ...props }) => {
    const getValue = (size) => {
        if (!size)
            return 100;
        ...
        const sizeToken = Array.from(defaultSystem.tokens.categoryMap.get("sizes")?.values() ?? []).find((token) => token.name === `sizes.${size}`);
        if (!sizeToken?.value)
            return 100;
    ...
    if (fill) {
        return (<ChakraImage asChild position="relative" width={width} height={height} {...props}>
        <NextImage src={src} alt={alt} fill={true} sizes={sizes}/>
      </ChakraImage>);
    }
    return (<ChakraImage asChild {...props}>
      <NextImage src={src} alt={alt} width={getValue(width)} height={getValue(height)}/>
    </ChakraImage>);
};
